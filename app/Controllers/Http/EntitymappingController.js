...
class EntitymappingController {
    ...
    async sourceEntities({ request, response, error }) {
        try {
            var sourcentities = await Database.select('*').from('PROJECT_SOURCE_ENTITY_LIST');
    ...
    async sourceEntityFields({ request, response, error }) {
        try {
            ...
            let qry = await Database.select('COLUMN_ID', 'COLUMN_NAME', 'DISPLAY_NAME')
    ...
    async destEntities({ request, response, error }) {
        try {
            var destentities = await Database.select('*').from('PROJ_DATATYPE_ENTITY_LIST');
    ...
    async destEntityFields({ request, response, error }) {
        try {
            var data = request.body;
            let qry = await Database.select('COLUMN_ID', 'COLUMN_NAME')
    ...
    async listofMapfeilds({request, response, error}){
        try{ 
            var data = request.body;
           let mapfields = await Database.select('*').from('PROJ_COLUMN_MAPPING')
    ...
    async removeMappedFields({request, response, error}){
        try{         
            var data = request.body;
             let deletedfields = await Database.table('PROJ_COLUMN_MAPPING')
            ...
            let qry3 = await Database.select('ENTITY_NAME').from('PROJECT_SOURCE_ENTITY_LIST').where('ENTITY_ID', data.sourceentityid);
            console.log(qry3);
             let transactions = await Database.insert({ PROJECT_ID:2,
                TRANSACTION_DATE:date,
                ENTITIY_ACCESSED:qry3[0].ENTITY_NAME,
    ...
    async deleteIndividualMappedField({request, response, error}){
        try{
            var data = request.body;
             let deletedfield = await Database.table('PROJ_COLUMN_MAPPING')
             .where({'SOURCE_ENTITY_ID': data.sourceentityid, 
             'DISPLAY_NAME':data.sourcecolumnname, 
             'DESTINATION_COLUMN_NAME':data.destinationcolumnname})
             .delete();
