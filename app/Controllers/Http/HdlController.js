...
const DataTransferRulesForDefaultTransfers = [
    ...
    //         // AND P.PERSON_NUMBER in (SELECT MAPPED_PERSON_NUMBER FROM VALIDATIONS_DATA)"
    ...
    //         //  WHERE pn.PERSON_NUMBER in (SELECT MAPPED_PERSON_NUMBER FROM VALIDATIONS_DATA)"
    ...
    //         //  where PLI.PERSON_NUMBER in (SELECT MAPPED_PERSON_NUMBER FROM VALIDATIONS_DATA)"
    // },
    ...
    //         // WHERE PERSON_NUMBER in (SELECT MAPPED_PERSON_NUMBER FROM VALIDATIONS_DATA)"
...

    {
        ...
        SourceQuery: "select Action_Code as ActionCode, Source_System_Owner as SourceSystemOwner," + " PERSON_NUMBER || '_' || 'ASG' \"SOURCESYSTEMID\"" + ",to_char(Effective_Start_Date,'YYYY/MM/DD') as EffectiveStartDate,to_char(Effective_End_Date, 'YYYY/MM/DD') as EffectiveEndDate, Effective_Sequence as EffectiveSequence,"
            ...
            + " PERSON_NUMBER || '_' || 'ETERM'   \"WTAIDSOURCESYSTEMID\"" + ",JobCode , Department_Name as DepartmentName, LocationCode , GradeCode " + " FROM ASSIGNMENT  WHERE PERSON_NUMBER in (SELECT MAPPED_PERSON_NUMBER FROM VALIDATIONS_DATA)" 
